...
def fetch_details_by_name():
    ...
    for result in results:
        ...
        row_label.pack()
    if not results:
        tk.messagebox.showinfo("No Results", f"No passengers are booked to travel under '{fname + lname}'.")


# Function to query traveling passengers
def fetch_details_by_date():
...
def fetch_details_by_age():
    ...
    for result in results:
        ...
        row_label.pack()
    if not results:
        tk.messagebox.showinfo("No Results", f"No passengers are in age range of '{max_age, min_age}'.")


def fetch_details_by_ticket_type():
    ...
    for result in results:
        ...
        row_label.pack()
    if not results:
        tk.messagebox.showinfo("No Results", f"No passengers have '{ticket_type}' type of ticket.")


def fetch_details_by_train_name():
    ...
    for result in results:
        ...
        row_label.pack()
    if not results:
        tk.messagebox.showinfo("No Results", f"No passengers have booked tickets for '{train_name}'.")
...
tab_parent.add(tab1, text="Retrieve Trains Booked by Passenger Name")
...
query_button = tk.Button(tab1, text="Result", command=fetch_details_by_name)
query_button.pack(padx=10, pady=10)




# Tab for retrieving booked trains details by passenger name
tab2 = ttk.Frame(tab_parent)
tab_parent.add(tab2, text="Retrieve Train's Details by Date")
...
# Creating the input fields for first name and last name
date_label = tk.Label(frame, text="Enter train's date (format -> yyyy-mm-dd):")
date_entry = tk.Entry(frame)

# Creating the button to execute the query
query_button = tk.Button(tab2, text="Result", command=fetch_details_by_date)
query_button.pack(padx=10, pady=10)
...
# Tab for retrieving details by age range
tab3 = ttk.Frame(tab_parent)
tab_parent.add(tab3, text="Retrieve All Details by Age range")
...
# Creating the button to execute the query
query_button = tk.Button(tab3, text="Result", command=fetch_details_by_age)
query_button.pack(padx=10, pady=10)

# Creating the text box to display the results


# Tab for retrieving booked trains details by ticket type
tab4 = ttk.Frame(tab_parent)
tab_parent.add(tab4, text="Retrieve Train's Details by Ticket Type")
...
query_button = tk.Button(tab4, text="Result", command=fetch_details_by_ticket_type)
query_button.pack(padx=10, pady=10)



# Tab for retrieving booked trains details by train name
...
query_button = tk.Button(tab5, text="Result", command=fetch_details_by_train_name)
query_button.pack(padx=10, pady=10)
